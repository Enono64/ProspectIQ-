...
export default function PlayerDetail() {
  ...
  async function handleInstatImport(stats, file) {
    try {
      // Sauvegarder les stats en priorité
      const updates = { ...stats, instat_imported_at: new Date().toISOString() }
      await api.updatePlayer(id, updates)
      
      // Upload fichier en arrière-plan (optionnel, ne bloque pas)
      if (file) {
        api.uploadInstatFile(id, file)
          .then(path => api.updatePlayer(id, { instat_file_path: path }))
          .catch(e => console.warn('Upload fichier:', e.message))
      }
